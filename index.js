...
// ─── STATE V2 ─────────────────────────────────────────────────────────────────
const v2ContractStates    = new Map();
// ─────────────────────────────────────────────────────────────────────────────
...
async function registerSlashCommands() {
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('contrat')
      .setDescription('(V2) Créer un nouveau contrat HEO')
      .toJSON(),
  ];
  const rest = new REST({ version: '10' }).setToken(CONFIG.TOKEN);
  try {
    await rest.put(Routes.applicationGuildCommands(CONFIG.CLIENT_ID, CONFIG.GUILD_ID), { body: commands });
    console.log('✅ Commandes slash enregistrées (v1 + v2)');
  } catch (err) {
...
// ─── HELPERS V2 ───────────────────────────────────────────────────────────────
async function v2HandleTransition(category, state, interaction) {
  const stepInfo = CONFIG.V2_STEPS[state.step];
  ...
  switch (state.step) {
    ...
    case 4:
      ...
      devChan?.send(`<@&${CONFIG.V2.ROLE_OWNER}> : Merci de payer le développeur.`);
      break;
    case 5:
      if (devChan) await devChan.setName(`✅-${devChan.name}`);
      const secTicket = await category.guild.channels.create({
        name: `📍💳-paiement-secrétaire`,
        parent: category.id,
        type: ChannelType.GuildText,
      });
      secTicket.send(`<@&${CONFIG.V2.ROLE_OWNER}> : Attente du paiement du secrétaire.`);
      break;
  }

  if (!interaction.replied && !interaction.deferred) {
    await interaction.reply({ content: `Passage à l'étape : **${stepInfo.label}**`, ephemeral: true });
  }
}
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'contrat') {
    const modal = new ModalBuilder().setCustomId('v2_modal_create').setTitle('📋 Nouveau Contrat (V2)');
    modal.addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v2_name').setLabel('Nom du contrat').setStyle(TextInputStyle.Short).setRequired(true)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v2_budget').setLabel('Budget (€)').setStyle(TextInputStyle.Short).setRequired(true)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v2_deadline').setLabel('Délai').setStyle(TextInputStyle.Short).setRequired(true)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v2_desc').setLabel('Description').setStyle(TextInputStyle.Paragraph).setRequired(true)),
    );
    return await interaction.showModal(modal);
  }

  // ── V2 : Modal création soumis ────────────────────────────────────────────
  if (interaction.isModalSubmit() && interaction.customId === 'v2_modal_create') {
    const name   = interaction.fields.getTextInputValue('v2_name');
    const budget = interaction.fields.getTextInputValue('v2_budget');
    ...
    const ticketClient = await interaction.guild.channels.create({
      ...
      permissionOverwrites: [
        ...
        { id: CONFIG.V2.ROLE_SECRETAIRE,      allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] },
        { id: CONFIG.V2.ROLE_OWNER,           allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] },
        { id: CONFIG.V2.ROLE_STAFF,           allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] },
      ],
    });
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('v2_btn_next').setLabel('Étape Suivante ➡️').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId('v2_btn_back').setLabel('🔙').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId('v2_btn_cancel').setLabel('🛑 Annuler').setStyle(ButtonStyle.Danger),
    );

    await ticketClient.send({ content: `<@&${CONFIG.V2.ROLE_SECRETAIRE}>`, embeds: [embed], components: [row] });

    v2ContractStates.set(category.id, {
    ...
    await interaction.reply({ content: `✅ Contrat V2 initialisé : ${ticketClient}`, ephemeral: true });
    return;
  }

  // ── V2 : Boutons ─────────────────────────────────────────────────────────
  if (interaction.isButton() && (
    interaction.customId === 'v2_btn_next' ||
    interaction.customId === 'v2_btn_back' ||
    interaction.customId === 'v2_btn_cancel'
  )) {
    const category = interaction.channel?.parent;
    if (!category) return;
    const state = v2ContractStates.get(category.id);
    if (!state) return;

    if (interaction.customId === 'v2_btn_next') {
      if (state.step === 0) {
        const modalDev = new ModalBuilder().setCustomId('v2_modal_dev').setTitle('Assigner le Développeur');
        modalDev.addComponents(
          new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v2_dev_tags').setLabel('Tags Dev (ex: @user1, @user2)').setStyle(TextInputStyle.Short).setRequired(true)),
          new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v2_infos').setLabel('Infos additionnelles').setStyle(TextInputStyle.Paragraph).setRequired(false)),
        );
        return await interaction.showModal(modalDev);
      }
      state.history.push(state.step);
      state.step++;
      await v2HandleTransition(category, state, interaction);
      return;
    }

    if (interaction.customId === 'v2_btn_back') {
      if (state.history.length === 0) {
        await interaction.reply({ content: 'Retour impossible.', ephemeral: true }); return;
      }
    ...
    if (interaction.customId === 'v2_btn_cancel') {
      state.isCancelled = !state.isCancelled;
      const prefix = state.isCancelled ? '🛑' : CONFIG.V2_STEPS[state.step].emoji;
  ...
  // ── V2 : Modal assignation dev soumis ────────────────────────────────────
  if (interaction.isModalSubmit() && interaction.customId === 'v2_modal_dev') {
    const category = interaction.channel?.parent;
    if (!category) return;
    const state = v2ContractStates.get(category.id);
    if (!state) return;
    ...
    state.history.push(state.step);
    state.step = 1;
    const devTags = interaction.fields.getTextInputValue('v2_dev_tags');
    ...
    await interaction.channel.send(`🛠️ Développeur choisi. Un message a été envoyé dans le salon dev.`);
    return;
  }
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'contrats') {
    ...
    if (tickets.length === 0) {
      await interaction.editReply({ content: '📭 Aucun contrat en cours.' }); return;
    }
  ...
  if (interaction.isButton() && interaction.customId === 'dev_form_ouvrir_modal') {
    ...
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('pseudos_assignes').setLabel('Pseudos des devs assignés au projet')
        .setStyle(TextInputStyle.Paragraph).setPlaceholder('Ex: Jean, Marc, Sophie\n(Sépare les pseudos par des virgules)').setRequired(true)
    ));
    await interaction.showModal(modal);
    return;
  }
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'dev_form_modal_assignes') {
    ...
    for (const type of pending.types) {
      ...
      for (const [, member] of guild.members.cache) {
        if (member.roles.cache.has(roleId) && !assignesIds.includes(member.id) && !backupIds.includes(member.id)) backupIds.push(member.id);
      }
    }
